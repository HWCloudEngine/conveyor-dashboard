...
from horizon.utils import urlresolvers

from openstack_dashboard import api
from openstack_dashboard.api import cinder
from openstack_dashboard import policy
from conveyordashboard.volumes.volumes import recover_utils
...
class LaunchVolume(tables.LinkAction):
    ...
    url = "horizon:conveyor:instances:launch"
...
class CreateVolume(tables.LinkAction):
    ...
    url = "horizon:conveyor:volumes:volumes:create"
...
class ExtendVolume(VolumePolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:conveyor:volumes:volumes:extend"
...
class EditAttachments(tables.LinkAction):
    ...
    url = "horizon:conveyor:volumes:volumes:attach"
    classes = ("ajax-modal",)
    icon = "pencil"
...
class CreateSnapshot(VolumePolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:conveyor:volumes:volumes:create_snapshot"
    classes = ("ajax-modal",)
    icon = "camera"
...
class CreateTransfer(VolumePolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:conveyor:volumes:volumes:create_transfer"
    classes = ("ajax-modal",)
...
class EditVolume(VolumePolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:conveyor:volumes:volumes:update"
    classes = ("ajax-modal",)
    icon = "pencil"
...
class RetypeVolume(VolumePolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:conveyor:volumes:volumes:retype"
    classes = ("ajax-modal",)
    icon = "pencil"
...
class AcceptTransfer(tables.LinkAction):
    ...
    url = "horizon:conveyor:volumes:volumes:accept_transfer"
    classes = ("ajax-modal",)
    icon = "exchange"
...
def get_attachment_name(request, attachment):
    ...
    try:
        url = reverse("horizon:clone:instances:detail", args=(server_id,))
...
def get_backup_zone(volume):
    return volume.backup_zone if hasattr(volume, 'backup_zone') else None
...
class VolumesTableBase(tables.DataTable):
    ...
    name = tables.Column("name",
                         verbose_name=_("Name"),
                         link="horizon:clone:volumes:volumes:detail")
...
def get_recovered_volume_link(volumesTable):
    link = None
    if hasattr(volumesTable, 'recovered_volume_id') and volumesTable.recovered_volume_id:
        link = urlresolvers.reverse("horizon:clone:volumes:volumes:detail",
                                    kwargs={'volume_id': volumesTable.recovered_volume_id})
    return link


def get_az_and_state(volume):
    return getattr(volume, "availability_zone", "None") + ":" + \
           getattr(volume, "availability_zone_state", "None")


class RecoverVolumeAction(tables.LinkAction):
    name = "recover_volume"
    verbose_name = _("Recover Volume")
    url = "horizon:clone:volumes:volumes:recover_volume"
    classes = ("ajax-modal",)
    icon = "pencil"
    policy_rules = (("volume", "volume:recover"),)

    def allowed(self, request, volume=None):
        return hasattr(volume, 'recover_status') and \
               volume.recover_status == 'Recoverable'


class RecoverVolumeBatchAction(VolumePolicyTargetMixin, BatchAction):
    name = "recover"

    def __init__(self, **kwargs):
        super(RecoverVolumeBatchAction, self).__init__(**kwargs)
        self.name = kwargs.get('name', self.name)
        if not hasattr(self, "action_present"):
            self.action_present = kwargs.get('action_present', _("Recover"))
        if not hasattr(self, "action_past"):
            self.action_past = kwargs.get('action_past', _("Recovered"))
        self.icon = "pencil"

    def action(self, request, obj_id):
        """Action entry point. Overrides base class' action method.

        Accepts a single object id passing it over to the recover method
        responsible for the object's destruction.
        """
        return self.recover(request, obj_id)
    ...
    def action_past(count):
        return ungettext_lazy(
            u"Scheduled recover of Volume",
            u"Scheduled recover of Volumes",
            count
        )

    policy_rules = (("volume", "volume:recover"),)

    def recover(self, request, obj_id):
        obj = self.table.get_object_by_id(obj_id)
        name = self.table.get_object_display(obj)
        recover_utils.recover_volume_from_volume_id(request, obj_id)

    def allowed(self, request, volume=None):
        if volume:
            return request.user.tenant_name == volume.tenant_name and \
                   volume.recover_status == "Recoverable"
        return True
...
class VolumesTable(VolumesTableBase):
    name = tables.Column("name",
                         verbose_name=_("Name"),
                         link="horizon:clone:volumes:volumes:detail")
    volume_type = tables.Column(recover_utils.get_generic_volume_type_from_volume,
                                verbose_name=_("Type"))
    attachments = AttachmentColumn("attachments",
                                   verbose_name=_("Attached To"))
    availability_zone = tables.Column("availability_zone",
                         verbose_name=_("Availability Zone"), classes=("az_field",))
    ...
    encryption = tables.Column(get_encrypted_value,
                               ...
                               link="horizon:clone:volumes:"
                                    "volumes:encryption_detail")'''
    backup_zone = tables.Column(get_backup_zone,
                              verbose_name=_("Backup Zone"), classes=("az_field",))
    availability_zone_and_state = tables.Column(get_az_and_state,
                                             verbose_name=_("Zone State"), classes=("nowrap-col","az_field"))

    recovered_volume_id = tables.Column("recovered_volume_id",
                                        hidden=True,
                                        verbose_name=_("Recovered Volume Id"))
    recover_status = tables.Column("recover_status",
                                   verbose_name=_("Recover Status"),
                                   link=get_recovered_volume_link)
    ...
    class Meta:
        ...
        row_actions = (EditVolume, ExtendVolume, LaunchVolume, EditAttachments,
                       ...
                       DeleteVolume, RecoverVolumeAction)
        columns = ('availability_zone_and_state',
                   'recover_status', 'recovered_id', 'backup_zone', 'tenant',
                   'name', 'size', 'status', 'volume_type', 'attachments')		
...
class DetachVolume(tables.BatchAction):
    ...
    def get_success_url(self, request):
        return reverse('horizon:clone:volumes:index')
