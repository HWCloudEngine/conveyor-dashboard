...
from openstack_dashboard.dashboards.clone.volumes \
...
from openstack_dashboard.dashboards.clone.volumes \
    .volumes import recover_utils as recover_utils
...
class JSONView(View):
    def get(self, request, *args, **kwargs):
        ...
        if not zones:
            zone_list = [{'id': "", 'name': "None"}]
        else:
            dc = request.GET.get("dc")
            if len(dc) > 0:
                zone_list = [{'id': zone.zoneName, 'name': zone.zoneName.split(".")[0]}
                              for zone in zones if zone.zoneState['available'] and forms.fields.get_dc(zone.zoneName) == dc]
            else:
                zone_list = [{'id': zone.zoneName, 'name': zone.zoneName.split(".")[0]}
                              for zone in zones if zone.zoneState['available']];
        # self._prepare_gateway_ports(data['routers'], data['ports'])
        json_string = json.dumps({'zones': zone_list}, ensure_ascii=False)
        return HttpResponse(json_string, content_type='text/json')

class DetailView(tabs.TabView):
    tab_group_class = project_tabs.VolumeDetailTabs
    template_name = 'clone/volumes/volumes/detail.html'
...
class CreateView(forms.ModalFormView):
    ...
    template_name = 'clone/volumes/volumes/create.html'
    submit_label = _("Create Volume")
    submit_url = reverse_lazy("horizon:clone:volumes:volumes:create")
    success_url = reverse_lazy('horizon:clone:volumes:volumes_tab')
...
class ExtendView(forms.ModalFormView):
    ...
    template_name = 'clone/volumes/volumes/extend.html'
    submit_label = _("Extend Volume")
    submit_url = "horizon:clone:volumes:volumes:extend"
    success_url = reverse_lazy("horizon:clone:volumes:index")
...
class CreateSnapshotView(forms.ModalFormView):
    ...
    template_name = 'clone/volumes/volumes/create_snapshot.html'
    submit_url = "horizon:clone:volumes:volumes:create_snapshot"
    success_url = reverse_lazy('horizon:clone:volumes:snapshots_tab')
...
class UploadToImageView(forms.ModalFormView):
    ...
    template_name = 'clone/volumes/volumes/upload_to_image.html'
    submit_label = _("Upload")
    submit_url = "horizon:clone:volumes:volumes:upload_to_image"
    success_url = reverse_lazy("horizon:clone:volumes:index")
...
class CreateTransferView(forms.ModalFormView):
    ...
    template_name = 'clone/volumes/volumes/create_transfer.html'
    success_url = reverse_lazy('horizon:clone:volumes:volumes_tab')
    modal_id = "create_volume_transfer_modal"
    modal_header = _("Create Volume Transfer")
    submit_label = _("Create Volume Transfer")
    submit_url = "horizon:clone:volumes:volumes:create_transfer"
...
class UpdateView(forms.ModalFormView):
    ...
    template_name = 'clone/volumes/volumes/update.html'
    submit_url = "horizon:clone:volumes:volumes:update"
    success_url = reverse_lazy("horizon:clone:volumes:index")
    ...
    def get_object(self):
        if not hasattr(self, "_object"):
            ...
            try:
                ...
                url = reverse('horizon:clone:volumes:index')
...
class EditAttachmentsView(tables.DataTableView, forms.ModalFormView):
    ...
    template_name = 'clone/volumes/volumes/attach.html'
    submit_url = "horizon:clone:volumes:volumes:attach"
    success_url = reverse_lazy("horizon:clone:volumes:index")
...
class RetypeView(forms.ModalFormView):
    ...
    template_name = 'clone/volumes/volumes/retype.html'
    submit_label = _("Change Volume Type")
    submit_url = "horizon:clone:volumes:volumes:retype"
    success_url = reverse_lazy("horizon:clone:volumes:index")
...
class EncryptionDetailView(generic.TemplateView):
    template_name = 'clone/volumes/volumes/encryption_detail.html'
...
class RecoverVolumeView(forms.ModalFormView):
    form_class = project_forms.RecoverVolume
    template_name = 'clone/volumes/volumes/recover_volume.html'
    submit_label = _("Recover volume")
    submit_url = "horizon:clone:volumes:volumes:recover_volume"
    success_url = reverse_lazy('horizon:clone:volumes:volumes_tab')

    def get_context_data(self, **kwargs):
        context = super(RecoverVolumeView, self).get_context_data(**kwargs)
        context["volume_id"] = self.kwargs['volume_id']
        return context
    ...
    def get_data(self):
        try:
            volume_id = self.kwargs['volume_id']
            volume = cinder.volume_get(self.request, volume_id)
            volume.backup_zone = recover_utils.\
                get_volume_backup_zone(self.request, volume)
        except Exception:
            exceptions.handle(self.request,
                              _('Unable to retrieve volume details.'),
                              redirect=self.success_url)
    ...
    def get_initial(self):
        volume = self.get_data()
        print "original volume:%s" % volume
        return {'volume_id': self.kwargs["volume_id"],
                'status': volume.status,
                'attachments': volume.attachments,
                'volume_name': volume.name,
                'backup_zone': volume.backup_zone,
                'volume_type': volume.volume_type}
