...
from openstack_dashboard.dashboards.project.volumes.volumes import recover_utils
...
class CreateForm(forms.SelfHandlingForm):
    ...
    def __init__(self, request, *args, **kwargs):
        ...
        self.name_type_dict = {}
        self.categorize_same_name_types(request)
        type_list = []
        for name in self.name_type_dict.keys():
            if name == 'hybrid':
                type_list.append((name, name + "(backup per 10 minutes)"))
            else:
                type_list.append((name, name))
        self.fields['type'].choices = [("", _("No volume type"))] + type_list
    ...
    def categorize_same_name_types(self, request):
        volume_types = cinder.volume_type_list(request)
        # deduplicate type name
        for volume_type in volume_types:
            name = volume_type.name.split('@')[0]
            if name not in self.name_type_dict:
                self.name_type_dict[name] = [volume_type]
            else:
                self.name_type_dict[name] += [volume_type]
        ...
        try:
            ...
            if availableGB < data['size']:
                ...
                error_message = _('You are already using all of your available'
                                  ' volumes.')
                raise ValidationError(error_message)
            # map stripped type name to actual type name (unique one)
            type_name = data['type']
            if data['type']:
                types = self.name_type_dict[data['type']]
                '''for volume_type in types:
                    if hasattr(volume_type, 'extra_specs') and \
                                    'volume_backend_name' in volume_type.extra_specs:
                        backend = volume_type.extra_specs['volume_backend_name']
                        # split backend fields as format type:support_az:backup_az
                        backend_fields = backend.split(':')
                        if len(backend_fields) >= 2 and backend_fields[1].find(az) >= 0:
                            type_name = volume_type.id
                            print "type:%s found actual type name:%s" % (data['type'], type_name)
                            break'''
                for volume_type in types:
                    if not az:
                        # any az, pick up any az and compose the volume type
                        # TODO: not through scheduler
                        zone_list = []
                        if cinder_az_supported(request):
                            try:
                                zones = api.cinder.availability_zone_list(request)
                        ...
                        print "az:%s" % az
                    print "vol type:%s" % volume_type.name
                    print "data type:%s" % data['type']
                    if az in volume_type.name and data['type'] in volume_type.name:
                        type_name = volume_type.id
                        print "type:%s found actual type name:%s" % (data['type'], type_name)
                        break
...
class RecoverVolume(forms.SelfHandlingForm):
    name = forms.CharField(max_length=255, label=_("Volume Name"))

    if recover_utils.VERSION == '1.0':
        snapshot_source = forms.ChoiceField(
            label=_("Latest Recover Time Point"),
            widget=forms.SelectWidget(
                attrs={'class': 'snapshot-selector'},
                data_attrs=('size', 'name'),
                transform=lambda x: "%s (%sGB)" % (x.name, x.size)),
        ...
        type = forms.ChoiceField(
            label=_("Recover Target Type: ec2 supported ONLY!"),
            required=False,
            widget=forms.Select(
                attrs={'class': 'switched',
                       'data-switch-on': 'source',
                       'data-source-snapshot_source': _('Type')}))
        attachment = forms.SimpleChoiceField(
            label=_("Recover attachment"),
            required=False,
            widget=forms.Select(
                attrs={'class': 'switched',
                       'data-switch-on': 'source',
                       'data-source-snapshot_source': _('Recover attachment')}))
    elif recover_utils.VERSION == '2.0':
        backup_source = forms.ChoiceField(
            label=_("Latest Recover Time Point"),
            widget=forms.SelectWidget(
                attrs={'class': 'backup-selector'},
                data_attrs=('volume_id', 'status')),
            required=False)

        type = forms.ChoiceField(
            label=_("Recover Target Type:"),
            required=False,
            widget=forms.Select(
                attrs={'class': 'switched',
                       'data-switch-on': 'source',
                       'data-source-backup_source': _('Type')}))
        ...
        attachment = forms.SimpleChoiceField(
            ...
            widget=forms.Select(
                attrs={'class': 'switched',
                       'data-switch-on': 'source',
                       'data-source-backup_source': _('Recover attachment')}))

    size = forms.IntegerField(min_value=1, initial=1, label=_("Size (GB)"))


    backup_zone = forms.SimpleChoiceField(
        label=_("Recover to Zone:"),
        required=False,
        widget=forms.Select(
            attrs={'class': 'switched',
                   'data-switch-on': 'source',
                   'data-source-snapshot_source': _('Recover to Zone:')}))

    def __init__(self, request, *args, **kwargs):
        super(RecoverVolume, self).__init__(request, *args, **kwargs)

        self.init_volume_types(request)
        self.fields['name'].initial = self.initial['volume_name'] + "-recovered"
        self.fields['backup_zone'].choices = [(self.initial['backup_zone'],
                                               self.initial['backup_zone'])]
        try:
            recover_points_available = False
            if recover_utils.VERSION == '1.0':
                recover_points_available = self.init_recover_points(request)
            elif recover_utils.VERSION == '2.0':
                recover_points_available = self.init_recover_points(request, source_type='backup')
            if not recover_points_available:
                choices = [('', _("No Recover Time Point"))]
                self.fields['snapshot_source'].choices = choices
                del self.fields['name']
                del self.fields['size']
                del self.fields['type']
                del self.fields['attachment']
                del self.fields['backup_zone']
            else:
                if not self.initial['attachments']:
                    del self.fields['attachment']
                else:
                    # if volume has attachment, recover it
                    # TODO: NOT support one volume attached to multiple VMs
                    attachments = [(attachment['server_id'],
                                    attachment['device'] + ":" +
                                    attachment['instance_name'] + "-recovered")
                                   for attachment in self.initial['attachments']]
                    self.fields['attachment'].choices = attachments
                    self.fields['attachment'].initial = attachments[0]
        except Exception:
            exceptions.handle(request, _("Unable to retrieve "
                                         "volume recover points."))

    def init_volume_types(self, request):
        volume_types = cinder.volume_type_list(request)
        if recover_utils.VERSION == '1.0':
            ...
            original_type = self.initial['volume_type']
            target_type = []
            for volume_type in volume_types:
                generic_type = recover_utils.get_generic_volume_type(
                    volume_type.name)
                original_generic_type = recover_utils.get_generic_volume_type(
                    original_type)
                if generic_type == original_generic_type:
                    if volume_type.name.find(self.initial['backup_zone']) >= 0:
                        target_type.append((volume_type.name, generic_type))
                        print "found volume type:%s" % target_type

            self.fields['type'].choices = target_type

    def init_recover_points(self, request, source_type='snapshot'):
        recover_points_available = False
        if source_type == 'snapshot':
            ...
            if snapshots:
                ...
                # only show latest snapshot
                self.fields['snapshot_source'].choices = \
                    [(snapshots[0].id, snapshots[0])]
                self.fields['size'].initial = snapshots[0].size
            recover_points_available = bool(snapshots)
        elif source_type == 'backup':
            all_backups = cinder.volume_backup_list(request)
            backups = []
            for backup in all_backups:
                if backup.volume_id == self.initial['volume_id'] and \
            ...
            if backups:
                backups = sorted(backups,
                                 key=attrgetter('created_at'),
                                 reverse=True)
                # only show latest backups
                self.fields['backup_source'].choices = \
                    [(backups[0].id, backups[0].name)]
                self.fields['size'].initial = backups[0].size

            recover_points_available = bool(backups)
        return recover_points_available

    def clean(self):
        cleaned_data = super(RecoverVolume, self).clean()
        print "enter clean"
        if recover_utils.VERSION == '1.0':
            if not cleaned_data.get('snapshot_source'):
                msg = _('Recover time point must be specified')
                self._errors['snapshot_source'] = self.error_class([msg])
        elif recover_utils.VERSION == '2.0':
            if not cleaned_data.get('backup_source'):
                msg = _('Recover time point must be specified')
                self._errors['snapshot_source'] = self.error_class([msg])
    ...
    def handle(self, request, data):
        try:
            print "enter handle recover form"
            usages = quotas.tenant_limit_usages(self.request)
            availableGB = usages['maxTotalVolumeGigabytes'] - \
                          usages['gigabytesUsed']
            availableVol = usages['maxTotalVolumes'] - usages['volumesUsed']
            attachment_id = data.get('attachment', None) or None
            recover_source_id = None
            recover_source_type = None
            if data.get("snapshot_source", None):
                # Create from Snapshot
                snapshot = self.get_snapshot(request,
                                             data["snapshot_source"])
                recover_source_id = snapshot.id
                recover_source_type = 'snapshot'
                if data['size'] < snapshot.size:
                    error_message = _('The volume size cannot be less than '
                                      'the snapshot size (%sGB)') % snapshot.size
                    raise ValidationError(error_message)
            elif data.get("backup_source", None):
                # Create from Snapshot
                backup = self.get_backup(request,
                                         data["backup_source"])
                recover_source_id = backup.id
                recover_source_type = 'backup'
                if data['size'] < backup.size:
                    error_message = _('The volume size cannot be less than '
                                      'the backup size (%sGB)') % backup.size
                    raise ValidationError(error_message)
            else:
                if type(data['size']) is str:
                    data['size'] = int(data['size'])
            ...
            # start recover volume according to its recover status
            device = None
            for attachment in self.initial['attachments']:
                if attachment['server_id'] == attachment_id:
                    device = attachment['device']
            return recover_utils.recover_volume(request, self.initial['volume_id'],
                                                self.initial['volume_name'],
                                                data['name'],
                                                data['size'],
                                                data['type'],
                                                recover_source_id,
                                                self.initial['backup_zone'],
                                                recover_source_type=recover_source_type,
                                                attachment_server_id=attachment_id,
            ...
            self.api_error(_("Unable to recover volume, message:%s" % e))
            return False
    ...
    @memoized
    def get_backup(self, request, id):
        return cinder.volume_backup_get(request, id)