...
from conveyordashboard.volumes.backups import tables as backups_tables
from conveyordashboard.volumes.snapshots import tables as vol_snapshot_tables
from conveyordashboard.volumes.volumes import tables as volume_tables
...
class VolumeTableMixIn(object):
    ...
    def _get_az_states(self):
        # TODO: get cinder service status from cinder
        # for now, only cascading nova implemented this status,
        # just check nova service status as az status
        try:
            services = api.nova.service_list(self.request)
            states = SortedDict([(service.zone, service.state)
                                 for service in services if service.binary == "nova-compute"])
            return states
        except Exception:
            exceptions.handle(self.request,
    ...
    def _set_az_state(self, volumes, states):
        for volume in volumes:
            az = volume.availability_zone
            volume.availability_zone_state = states.get(az)
...
class VolumeTab(tabs.TableTab, VolumeTableMixIn):
    ...
    def get_volumes_data(self):
        ...
        az_states = self._get_az_states()
        self._set_az_state(volumes, az_states)
        if recover_utils.VERSION == '1.0':
            recover_utils.set_readable_volume_recover_status(self.request,
                                                             volumes)
        elif recover_utils.VERSION == '2.0':
            recover_utils.set_readable_volume_recover_status(self.request,
                                                             volumes,
                                                             source_type='backup')
        recover_utils.set_volume_backup_zone_from_type(self.request, volumes)
        return volumes
