...
class ResourceDetailFromPlan(object):
    ...
    def _render_qos(self, context):
        props = context['data']
        specs = '\n'.join(['%s=%s' % (k, v) for k, v in props['specs'].items()])
        props['specs'] = specs
        return loader.render_to_string(self.container, context)
    ...
    def _render_net(self, context):
        ...
        if 'router_external' not in val_specs and 'router:external' in val_specs:
            val_specs['router_external'] = val_specs['router:external']
        if 'segmentation_id' not in val_specs and 'provider:segmentation_id' in val_specs:
            val_specs['segmentation_id'] = val_specs['provider:segmentation_id']
        if 'physical_network' not in val_specs and 'provider:physical_network' in val_specs:
            val_specs['physical_network'] = val_specs['provider:physical_network']
        if 'network_type' not in val_specs and 'provider:network_type' in val_specs:
            val_specs['network_type'] = val_specs['provider:network_type']
        props['admin_state_up'] = strutils.bool_from_string(props['admin_state_up'])
    ...
    def _render_subnet(self, context):
        ...
        #                                          excepts=[self.res_id])
        # else:
        #     dep_nets = search_dependent_items(copy.deepcopy(dependencies),
        #                                       [self.res_id],
        #                                       'network')
        #     dep_subnets = search_dependent_items(copy.deepcopy(dependencies),
        #                                          dep_nets,
    ...
    def _render_port(self, context):
        plan = api.plan_get(self.request, self.plan_id)
        # LOG.info("Port==>Plan: %s", plan.__dict__)
...
#                 subnet_id = dependencies[res_id_subnet]["id"]
        ...
        if rules:
            if isinstance(rules, six.string_types):
                ...
                for r in rules:
                    if 'id' not in r:
                        r['id'] = str(uuid.uuid4())
            tmp_rs =[os_api.neutron.SecurityGroupRule(rebuild_rules(r)) for r in rules]
            props['rules'] = json.dumps(rules)
            rules_table = topology_tables.RulesTable(self.request, tmp_rs, secgroup_id=context['id'])
    ...
    def render(self):
        ...
        context = {'type': node_type,
                   ...
                   'id': resource.get('id', None) or resource['extra_properties']['id'],
...
class PlanUpdate(object):
    ...
    def _update_volume_with_vt(self, new_vol, dep_items):
        ...
        if not new_vol['volume_type']:
            for key, value in dep_items.items():
                ...
                if len([k for k, v in self.dependencies.items() if k != res_id and key in v['dependencies']]) == 0:
                    data = {
            ...
            for key, value in dep_items.items():
                ...
                if item is not None:
                    ...
                    if self.dependencies[vt_tmpl_name].get(ACTION_KEY,
                                                           '') == ACTION_DELETE:
                        LOG.debug(
                        ...
                        if len(vt_deps):
                            for qos_id in vt_deps.keys():
                                if self.dependencies[qos_id].get(ACTION_KEY,
                                                                 None) == ACTION_DELETE:
                                    LOG.debug(
                                        "Remove action 'delete' flag for qos: %s",
                                        qos_id)
                    ...
                    if not len([k1 for k1, v1 in self.dependencies.items() if key in v1['dependencies']]):
                        data = {
                            ACTION_KEY: ACTION_DELETE,
                            TAG_RES_TYPE: value['type'],
                            TAG_RES_ID: key,
                ...
                # The new vt does not exist in plan.
                    ...
                    # Add new volume type.
                    vt_res, vt_dep = build_volumetype(new_vt, props=vt_props, deps=vt_deps)
    ...
    def _update_volume_without_vt(self, new_vol):
        ...
        if new_vol['volume_type']:
            ...
            if existed:
                ...
                # Remove ACTION 'delete' flag for volum volume type.
                if self.dependencies[vt_tmpl_name].get(ACTION_KEY, '') == ACTION_DELETE:
                    LOG.debug("Remove action 'delete' flag for volume_type: %s", vt_tmpl_name)
                    self.dependencies[vt_tmpl_name].pop(ACTION_KEY, None)
                    self.updated_resources[vt_tmpl_name].pop(ACTION_KEY)
                    ...
                    if len(vt_deps):
                        for qos_id in vt_deps.keys():
                            if self.dependencies[qos_id].get(ACTION_KEY, None) == ACTION_DELETE:
                                LOG.debug("Remove action 'delete' flag for qos: %s", qos_id)
                                self.dependencies[qos_id].pop(ACTION_KEY)
                ...
                vt_props = {}
                if new_qos_id:
                    existed, qos_dep = self._check_existed_from_deps(new_qos_id)
                    if existed:
                        # TODO
    ...
    def _update_volume(self):
        ...
        # Check dependencies
        dependent_items = self._dependent_items(dep_type=consts.CINDER_VOL_TYPE)
        ...
        if not qos_id:
            ...
            for key, value in dep_items.items():
                ...
                if item is not None:
                    ...
                    if self.dependencies[tmpl_name].get(ACTION_KEY, '') == ACTION_DELETE:
                ...
                # The new qos does not exist in plan.
                    ...
                    # Add dependencies for volume.
                    self.dependencies[res_id]['dependencies'].remove(key)
                    self.dependencies[res_id].get('dependencies', {}).append(
                        qos_tmp_name)
    ...
    def _update_volumetype_without_qos(self, vt):
        ...
        if qos_id:
            ...
            if existed:
                ...
                if self.dependencies[qos_tmpl_name].get(ACTION_KEY, '') == ACTION_DELETE:
    ...
    def _update_volumetype(self):
        ...
        if existed:
            ...
            LOG.debug("Update dependent volumes for volume_type: %s", new_vt_tmpl_name)
            ...
            if len(dep_vols):
                ...
                for dep_vol_id, dep_vol in dep_vols.items():
                    self.dependencies[dep_vol_id]['dependencies'].remove(res_id)
                    self.dependencies[dep_vol_id]['dependencies'].append(new_vt_tmpl_name)
                    t_dep = {'volume_type': {'get_resource': new_vt_tmpl_name}}
                    self.updated_resources[dep_vol_id].update({TAG_UPDATED: True})
                    if dep_vol_id in self.update_resource:
                        self.update_resource[dep_vol_id].get('properties', {}).update(t_dep)
                    else:
                        update_data = copy.deepcopy(self.updated_resources[dep_vol_id])
                        update_data.update({
            ...
            if self.dependencies[new_vt_tmpl_name].get(ACTION_KEY, '') == ACTION_DELETE:
                LOG.debug("Remove action 'delete' flag for volume_type: %s", new_vt_tmpl_name)
                ...
                dep_qoses = self._dependent_items(res_id=new_vt_tmpl_name, res_type=consts.CINDER_VOL_TYPE, dep_type=[consts.CINDER_QOS])
                if len(dep_qoses):
                    for qos_id in dep_qoses.keys():
                        if self.dependencies[qos_id].get(ACTION_KEY, None) == ACTION_DELETE:
                            LOG.debug("Remove action 'delete' flag for qos: %s", qos_id)
                            self.dependencies[qos_id].pop(ACTION_KEY, None)
                            self.updated_resources[qos_id].pop(ACTION_KEY, None)
            ...
            dependent_items = self._dependent_items()
            if len(dependent_items):
                return self._update_volumetype_with_qos(new_vt, dependent_items)
            else:
