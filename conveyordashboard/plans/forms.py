...
def preprocess_update_resources(update_resources):
    for res in update_resources:
        if res.get(constants.RES_ACTION_KEY, '') in (constants.ACTION_DELETE, constants.ACTION_ADD):
            update_resources.remove(res)
        else:
            res[constants.RES_ACTION_KEY] = constants.ACTION_EDIT

    for res in update_resources:
        # if constants.RES_ACTION_KEY not in res:
        #     res[constants.RES_ACTION_KEY] = constants.ACTION_EDIT
        # elif res[constants.RES_ACTION_KEY] in (constants.ACTION_DELETE,
        ...
        if res[TAG_RES_TYPE] == constants.NOVA_SERVER:
            ...
            if 'allocation_pools' in res and isinstance(res['allocation_pools'], six.string_types):
                pools = [dict(zip(['start', 'end'], pool.strip().split(',')))
                         for pool in res['allocation_pools'].split('\n')
                         if pool.strip()]
                res['allocation_pools'] = pools
            if 'host_routes' in res and isinstance(res['host_routes'], six.string_types):
                routes = [dict(zip(['destination', 'nexthop'],
                                   route.strip().split(',')))
                          for route in res['host_routes'].split('\n')
                          if route.strip()]
                res['host_routes'] = routes
            if 'dns_nameservers' in res and isinstance(res['dns_nameservers'], six.string_types):
            ...
            if 'value_specs' in res:
                ...
                if 'router_external' in val_specs:
                    specs['router:external'] = strutils.bool_from_string(val_specs.pop('router_external'))
                if 'segmentation_id' in val_specs:
                    specs['provider:segmentation_id'] = int(val_specs.pop('segmentation_id'))
                if 'physical_network' in val_specs:
                    specs['provider:physical_network'] = val_specs.pop('physical_network')
                if 'network_type' in val_specs:
                    specs['provider:network_type'] = val_specs.pop('network_type')
                res['value_specs'] = specs
            if 'admin_state_up' in res:
                res['admin_state_up'] = strutils.bool_from_string(res['admin_state_up'])
        elif res[TAG_RES_TYPE] == constants.NEUTRON_SECGROUP:
            if 'rules' in res:
                rules = res['rules']
                LOG.info("pro rules: {0} type: {1}".format(rules, type(rules)))
                if isinstance(rules, six.string_types):
                    rules = json.JSONDecoder().decode(rules)
                LOG.info("prepro rules: {0} type: {1}".format(rules, type(rules)))
                for r in rules:
                    r.pop('id', None)
                res['rules'] = rules
        # res.pop(TAG_RES_TYPE)
...
def clone_plan(request, action_type):
    ...
    if action_type == 'clone':
        ...
        sys_clone = strutils.bool_from_string(POST.get('sys_clone', 'off'),
                                              'off')
        api.export_clone_template(request, plan_id, sys_clone=sys_clone)
        api.clone(request, plan_id, az)
