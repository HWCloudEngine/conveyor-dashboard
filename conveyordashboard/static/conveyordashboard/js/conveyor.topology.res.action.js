	...
	$.each(data, function (k, v) {
		if(k=='name'){
			...
			$("#conveyor_plan_topology .node").each(function(){
				console.log($(this).context.__data__.info_box);
		...
		if(resource_type === 'OS::Nova::Server'){
			...
			for(var key in res_items[resource_type]) {
				...
				if(key==ipt){
					...
					for(var index in ipt_items){
						var node_name = ipt_items[index];
						var ipt_node = $("#id_"+node_name); if($(ipt_node).val()!=$(ipt_node).attr("data-ori")){data[node_name]=$(ipt_node).val()}
					}
				}else if(key==slt){
					var slt_items = value[slt];
					for(var index in slt_items){
						var node_name = slt_items[index];
						var slt_node = $("select[name="+node_name+"s]"); if($(slt_node).val()!=$(slt_node).attr("data-ori")){data[node_name]=$(slt_node).val()}
					}
				}else if(key==slt_from_existed){
					var slt_items = value[slt_from_existed];
					for(var index in slt_items){
						var node_name = slt_items[index];
						var slt_node = $("select[name="+node_name+"]"); if($(slt_node).val()!=$(slt_node).attr("data-ori")){data["id"]=$(slt_node).val();need_posted=true;}
					}
				}
			}
		}
...
$node_click = function(){
	...
	jQuery.post("/conveyor/plans/get_resource_detail",postdata,function(data){
		if(data.msg !== "success") {
			return false;
		}
...
function add_sg_rule_for_sg(){
	...
	$.ajaxSetup({beforeSend: function(xhr, settings){xhr.setRequestHeader("X-CSRFToken", $.cookie('csrftoken'));}});
	jQuery.post("/conveyor/plans/secgroup/"+$("#secgroup_wrap").attr('od_id')+"/create_rule/",{'secgroup_rule': JSON.stringify(secgroup_rule)},function(data){
