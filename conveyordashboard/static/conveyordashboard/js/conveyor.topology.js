...
function build_reverse_links(node){
  for (var i=0;i<nodes.length;i++){
    if(node.name ==='conveyor-qos'){console.log('qos-->'+nodes[i].id)}
    if(nodes[i].required_by){
      for (var j=0;j<nodes[i].required_by.length;j++){
        var dependency = nodes[i].required_by[j];
        if(node.name ==='conveyor-qos'){console.log('qos-->'+nodes[i].id+' required_by '+dependency)}
...
function update_node_name(id, name) {
  node = findNode(id);
  console.log('old name: '+node.name)
