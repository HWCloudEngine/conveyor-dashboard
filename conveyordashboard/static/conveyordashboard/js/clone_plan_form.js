...
$(function () {
  ...
  if (!window.conveyor.cancel_clone) {
    window.conveyor.cancel_clone = function () {
      var plan_id = $('#clone_plan_form').find('[name=plan_id]').val();
  ...
  function conveyorClonePlan(modal) {
    $(modal).find('.modal-footer a.cancel_plan').click(window.conveyor.cancel_clone);
    // $(modal).find('a.close').click(window.conveyor.cancel_clone);
  }

  conveyorClonePlan($('#clone_plan_form').parent());
});
