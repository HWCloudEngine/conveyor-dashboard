...
$(function () {
  ...
  function conveyorPlanDestination(modal) {
    ...
    if (clonePlanForm) {
      $(destinationForm).find('.modal-footer [type=submit]').click(function () {
        ...
        console.log('clone sys_clone: ' + $(destinationForm).find('[name=sys_clone]').val());
        $(clonePlanForm).find('[name=sys_clone]').val($(destinationForm).find('[name=sys_clone]').val());
  ...
  conveyorPlanDestination($('#destination_form').parent());
});
