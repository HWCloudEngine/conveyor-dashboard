...
class AddRuleView(forms.ModalFormView):
    ...
    def get_form_kwargs(self):
        ...
        try:
            groups = os_api.network.security_group_list(self.request)
        except Exception:
            groups = []
...
def clean_deps(deps):
    for res_id, dep in deps.items():
        dep['dependencies'] = [dep_id for dep_id in dep['dependencies'] if dep_id in deps]
...
class LocalTopologyView(View):
    ...
    def post(request, **kwargs):
        ...
        params = dict([(p.split('=')[0],p.split('=')[1]) for p in param.split('&')])
