...
class IndexView(views.HorizonTemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # plan_name = self.request.user.user_id + availability_zone
        LOG.info("User: %s", self.request.user.__dict__)

        servers = self._get_instances_data()
        servers_id_dict = self._extract_server_ids(servers, availability_zone)

        if not servers_id_dict:
            context['has_valid_res'] = False
            return context
        else:
            context['has_valid_res'] = True
